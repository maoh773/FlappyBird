...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let bgm = SKAudioNode(fileNamed: "bgm_game_playing")
    let seGameOver = SKAudioNode(fileNamed: "xxxx")
    let seItemGet = SKAudioNode(fileNamed: "xxx")
    ...
    override func didMove(to view: SKView) {
        ...
        seGameOver.autoplayLooped = false
        seItemGet.autoplayLooped = false
    ...
    func setupItem() {
        var itemImage = UIImage(named:"item")!
        itemImage = itemImage.resize(image: itemImage, width: 50.0)
        
        let itemTexture = SKTexture(image: itemImage)
        ...
        // 移動する距離を計算
        let movingDistance = CGFloat((self.frame.size.width + itemTexture.size().width / 2) * 2)
        
        // 画面外まで移動するアクションを作成,スクロールの速さを壁の移動と合わせないと死ぬ
        // 壁の移動距離は(画面サイズ + 壁の横幅 / 2) なのでその２倍の移動距離をとって移動速度も２倍にして調整
        ...
        let createItemAnimation = SKAction.run ({
            /**
             壁の移動速度から壁と壁の間隔は画面の横幅の約半分になるので、
             アイテムの位置は、(画面サイズ + 壁の横幅 / 2) + 画面の15％〜35％の間でランダム生成した値で決める
             これで壁と壁の間にアイテムが配置される
            **/
            let item = SKNode()
            item.zPosition = -25 // 雲、地面、壁より手前、
            
            let random_x_range = CGFloat.random(in: self.frame.size.width * 0.15..<self.frame.size.width * 0.35)
            item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2 + random_x_range, y: 0)
            
            let itemSprite = SKSpriteNode(texture: itemTexture)
            let random_y_range = CGFloat.random(in: self.frame.size.height * 0.4..<self.frame.size.height * 0.6)
            
            itemSprite.position = CGPoint(x: 0, y: random_y_range)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory ||
            ...
            //パーティクルを作成する。
            let particle = SKEmitterNode(fileNamed: "ItemGetParticle.sks")
